...
public class ParameterValidation {
   ...
   * @param isPositive {@link Predicate} evaluating to true when T is > 0
   * @param <T> staLength and ltaLength type
   * @throws IllegalArgumentException if staLength <= 0; if ltaLength <= 0;
   */
  ...
  /**
   * Checks the trigger and detrigger thresholds are both > 0
   * @param triggerThreshold STA/LTA trigger threshold
   * @param detriggerThreshold STA/LTA detrigger threshold
   * @throws IllegalArgumentException if triggerThreshold or detriggerThreshold are <= 0
   */
