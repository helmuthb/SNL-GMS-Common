...
public class SignalDetectorControlRouteHandler {
  ...
  public StandardResponse streaming(ContentType requestBodyType, byte[] body,
    ...
    logger
        .info(
            "Invoked Signal Detector Control / Streaming with Content-Type: {}, Accept: {}, and body: {}",
            requestBodyType, requestBodyType, body);
  ...
  private ExecuteStreamingCommand executeCommandFrom(StreamingDto streamingDto) {
    return ExecuteStreamingCommand
        .create(streamingDto.getChannelSegment(),
            ...
            //streamingDto.getSignalDetectorParameters(),
   ...
   * @return whether contentType can be accepted and returned by the streaming streaming detector service
   */
  ...
  public StandardResponse claimCheck(ContentType requestBodyType, byte[] body,
    ...
    logger.info(
        "Invoked Signal Detector Control / Claim Check with Content-Type: {}, Accept: {}, and body: {}",
        requestBodyType, requestBodyType, body);
    ...
    return StandardResponse.create(HttpStatus.OK_200,
        getSerializationOp(responseBodyType)
            .apply(signalDetectorControl.execute(claimCheckCommand)),
        responseBodyType);
  }
   ...
   * @return ExecuteStreamingCommand, not null
   */
  private ExecuteClaimCheckCommand executeCommandFrom(ClaimCheckDto claimCheckDto) {
    return ExecuteClaimCheckCommand
        .create(claimCheckDto.getStationId(),
  ...
  private static boolean isAcceptableClaimCheckResponseType(ContentType contentType) {
    return contentType == ContentType.APPLICATION_JSON || contentType == ContentType.APPLICATION_ANY;
  }
