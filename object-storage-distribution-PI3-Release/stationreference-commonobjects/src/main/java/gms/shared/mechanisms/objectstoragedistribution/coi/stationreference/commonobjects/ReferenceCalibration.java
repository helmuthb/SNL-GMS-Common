...
/**
 * As with the manufacturer-provided calibration information, this calibration information
 * is stored in the Calibration and Response classes. Calibration information is used to
 * convert the output of the instrument (e.g., volts, counts) into the phenomenon that the
 * instrument is measuring (e.g., seismic ground displacement). The Calibration class includes
 * information about when an instrument was calibrated (actual change time) and what the
 * response (calibration factor) was for a particular calibration period (i.e., inverse of
 * frequency). Response includes the full response function across a range of
 * periods/frequencies. The actual change time attribute in both the Calibration and Response
 * classes captures when the calibration was actually performed, and both classes also include
 * system change time as attributes, in order to track when the response information was
...
public final class ReferenceCalibration {
  ...
  public static ReferenceCalibration create(UUID channelId, double calibrationInterval,
      ...
      double timeShift, Instant actualTime, InformationSource informationSource,
      String comment) {

    return new ReferenceCalibration(UUID.randomUUID(), channelId, calibrationInterval, calibrationFactor,
        calibrationFactorError, calibrationPeriod, timeShift,
        actualTime, Instant.now(), informationSource, comment);
  }
   ...
   * Private constructor.
   */
  private ReferenceCalibration(UUID id, UUID channelId, double calibrationInterval, double calibrationFactor,
      double calibrationFactorError, double calibrationPeriod, double timeShift,
      Instant actualTime, Instant systemTime, InformationSource informationSource, String comment)
