...
public final class ReferenceNetworkMembership {
  ...
  public static ReferenceNetworkMembership create(String comment, Instant actualChangeTime,
    ...
    return new ReferenceNetworkMembership(UUID.randomUUID(), comment, actualChangeTime,
        Instant.now(), networkId, stationId, status);
  }
   ...
   * @param stationId
   * @param status
   * @return a new ReferenceNetworkMembership object from existing data
   * @throws NullPointerException
   */
  ...
  /**
   * Compare two membership records, if the network ID and the station ID match,
