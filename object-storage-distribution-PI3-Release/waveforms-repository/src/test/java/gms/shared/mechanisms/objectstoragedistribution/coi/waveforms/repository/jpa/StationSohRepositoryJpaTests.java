...
public class StationSohRepositoryJpaTests {
  ...
  // change this to "integrationDB" to run test against actual SQL instance
  private static final String persistenceUnit = "waveforms-unitDB";
  ...
  @Before
  public void setUp() {
    entityManagerFactory = WaveformsEntityManagerFactories.create(persistenceUnit);
  ...
  public void storeAndRetrieveAnalogSohTest() throws Exception {
    ...
    List<AcquiredChannelSohAnalog> list = (List<AcquiredChannelSohAnalog>) stationSohPersistenceJpa
        .retrieveAllAnalogSoh();
    assertNotNull(list);
    assertTrue(list.size() > 0);

    // Check that the last item added is identical to the test fixture.
    assertEquals(list.get(list.size() - 1), TestFixtures.channelSohAnalog);
  ...
  public void storeAndRetrieveBooleanSohTest() throws Exception {
    ...
    // Retrieve all records in the SOH table, there should be some.
    List<AcquiredChannelSohBoolean> list = (List<AcquiredChannelSohBoolean>) stationSohPersistenceJpa
        .retrieveAllBooleanSoh();
    assertTrue(list.size() > 0);

    // Check that the last item added is identical to the test fixture.
    assertEquals(list.get(list.size() - 1), TestFixtures.channelSohBool);
  }
  ...
  private <T extends AcquiredChannelSoh> void testSohQueryByProcessingChannelAndTimeRange(
    ...
    results = query.execute(
        expectedResult.getProcessingChannelId(),
