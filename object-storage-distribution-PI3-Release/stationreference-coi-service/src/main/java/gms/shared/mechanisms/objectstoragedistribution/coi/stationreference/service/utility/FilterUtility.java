...
public class FilterUtility {
  ...
  /**
   * Filters a list by start time to include only ones active at that start time
   * or later.  To do this, the latest time t for any element in elems is found
   * and then the elements are filtered by that time.  The elements are grouped together
   * (using the 'grouper' function) so that related entities are filtered against each other;
   * this is necessary since versions through time supersede previous ones of the same entity.
   * The resulting list will be returned sorted by time, using the given timeExtractor.
   * @param elems the list of elements to filter
   * @param startTime the start of the time range
   * @param timeExtractor a function which extracts an Instant from an element
   * (e.g. ReferenceNetwork::getActualChangeTime)
   * @param grouper a function to group related entities together so they can be filtered against each other
   * (e.g. ReferenceNetwork::getEntityId)
   * @param <T> the type of the elements to filter
   * @param <K> the type that groups related entities together (the return type of the 'grouper' function);
   * typically UUID.
  ...
  /**
   * Filters a list by end time to include only ones active up to that time.
   * The elements are grouped together (using the 'grouper' function)
   * so that related entities are filtered against each other.
   * The resulting list will be returned sorted by time, using the given timeExtractor.
   * @param elems the list of elements to filter
  ...
  /**
   * Filters a list by their times.  First, the latest time value t for an element in elems
   * is found for which t <= rangeStart and t is maximized.  Then, this t value is used
   * as the new time to filter the elements by.  This filters the elements by time assuming
