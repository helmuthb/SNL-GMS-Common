...
public class Cd11ConnectionManager extends GracefulThread {
  ...
  private final StationReceiverOsdGatewayInterface osdGateway; // TODO: In the future, this will be used to query the OSD for connection info.
  ...
  public Cd11ConnectionManager(Cd11ConnectionManagerConfig config) {
    ...
    this.osdGateway = new StationReceiverOsdGatewayAccessLibrary(
  ...
  protected void onStart() {
    ...
    try {
      serverSocket = new ServerSocket(
          config.connectionManagerPort, 0,
          InetAddresses.forString(config.connectionManagerIpAddress));
    } catch (IOException e) {
      logger.error(String.format(
          "Error binding to socket on IP/remotePort + %1$s:%2$d.",
  ...
  private void InitializeCd11StationsLookup() {
    ...
    // NOTE: Use the osdGateway object that already exists to query the OSD.
