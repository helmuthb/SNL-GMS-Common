...
public class Cd11RawStationDataFrameReader {
   ...
   * @param ci the creation info to associate with the returned data
   * @param idRetriever a function for retrieving channel ID given two strings and an Instant
   * (e.g. siteName, channelName, and time)
   * @return a pair of Channel segments and SOH's
   */
  public static Pair<List<ChannelSegment>, List<AcquiredChannelSoh>> read(
      RawStationDataFrame rsdf, CreationInfo ci,
      TriFunction<String, String, Instant, Optional<UUID>> idRetriever) throws Exception {
    Validate.notNull(rsdf);
    Validate.notNull(idRetriever);
    ...
    List<ChannelSegment> channelSegments = new ArrayList<>();
    ...
    for (int i = 0; i < df.channelSubframes.length; i++) {
      ...
      Optional<UUID> chanIdOptional = idRetriever.apply(sf.siteName, sf.channelName, sf.timeStamp);
      if (chanIdOptional.isPresent()) {
        Instant startTime = sf.timeStamp;
        Instant endTime = startTime.plusMillis(sf.subframeTimeLength);

        //Calculate sample rate
        double samples = sf.samples;
        double subframeTimeLength = sf.subframeTimeLength;
        int sampleRate = (int) Math.ceil(samples / subframeTimeLength
            * 1000); //time length is in milis, need to convert to seconds
        ...
        if (sf.compressionFormat == CompressionFormat.NONE) {
          waveformValues = Arrays
              .stream(WaveformReader.readSamples(waveformData, sf.dataType.name(),(int)samples,0)).asDoubleStream()
              .toArray();
        }
        //Canadian Compression, ignore data type field
          ...
          waveformValues = Arrays
              .stream(WaveformReader.readSamples(waveformData, "cc", (int)samples,0)).asDoubleStream()
              .toArray();
        } else {
