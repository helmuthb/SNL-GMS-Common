...
public class WaveformSpike3PtInterpreterTest {
  ...
  private static ChannelSegment noSpike;
  private static ChannelSegment oneSpike;
  private static ChannelSegment twoSpike;
  private static Waveform firstSpike;
  private static ChannelSegment endPointTest;
  private static ChannelSegment endPointTestGap;
  private static ChannelSegment sawtooth;
  ...
  public static void setUp() throws Exception {
    ...
    Waveform sawToothWf = Waveform
        .withInferredEndTime(startTime, samplesPerSec, sawToothValues.length, sawToothValues);
    sawtooth = TestUtility.createChannelSegment(List.of(sawToothWf));
    sawtoothTime = sawToothWf.timeForSample(200);
  }
  ...
  public void testCreateWaveformSpike3PtQcMasksOneSpike() {
    ...
    assertEquals(firstSpike.timeForSample(20), masks.get(0).getStartTime());

    //Check the QcMask time is the same as expected end time
    assertEquals(firstSpike.timeForSample(20), masks.get(0).getEndTime());
    ...
    assertEquals(oneSpike.getProcessingChannelId(), masks.get(0).getChannelId());
  }
  ...
  public void testCreateWaveformSpike3PtQcMasksTwoSpike() {
    ...
    SortedSet<Waveform> waveforms = twoSpike.getWaveforms();
    ...
    //Check the QcMask time is the same as expected start time
    assertEquals(waveform.timeForSample(20), masks.get(0).getStartTime());

    //Check the QcMask time is the same as expected end time
    assertEquals(waveform.timeForSample(20), masks.get(0).getEndTime());

    //Check the QcMask time is the same as expected start time
    assertEquals(waveform.timeForSample(30), masks.get(1).getStartTime());

    //Check the QcMask time is the same as expected end time
    assertEquals(waveform.timeForSample(30), masks.get(1).getEndTime());
  }
  ...
  public void testCreateNullChannelSegmentExpectNullPointerException() throws Exception {
    ...
    (new WaveformSpike3PtInterpreter())
        .createWaveformSpike3PtQcMasks(null, spikeThreshold, rmsLeadDifferences, rmsLagDifferences,
            rmsAmplitudeRatioThreshold);
  }
  ...
  @Test
  public void testCreateLeadSamplesNegativeExpectIllegalArgumentException() throws Exception {
    exception.expect(IllegalArgumentException.class);
    exception
        .expectMessage(
  ...
  @Test
  public void testCreateLagSamplesNegativeExpectIllegalArgumentException() throws Exception {
    exception.expect(IllegalArgumentException.class);
    exception
        .expectMessage(
  ...
  @Test
  public void testCreateNoLeadLagSamplesNegativeExpectIllegalArgumentException() throws Exception {
    exception.expect(IllegalArgumentException.class);
    exception
        .expectMessage(
  ...
  public void testCreateRmsAmplitudeRatioThresholdOneExpectIllegalArgumentException()
    ...
    exception
        .expectMessage(
            "WaveformSpike3PtInterpreter.createWaveformSpike3PtQcMasks requires rmsAmplitudeRatioThreshold > 1");
    (new WaveformSpike3PtInterpreter())
        .createWaveformSpike3PtQcMasks(noSpike, spikeThreshold, rmsLeadDifferences,
  ...
  /**
   * This test will find two spikes: one on the last point of the first waveform and another on
   * the first point of the second waveform.
   */
  @Test
  public void testCreateEndPointMasks() {
    ...
    //Retrieve collection of waveforms
    SortedSet<Waveform> waveforms = endPointTest.getWaveforms();
    ...
    Waveform waveform = it.next();
    final Instant firstMaskTime = waveform.timeForSample(waveform.getSampleCount() - 2);
  ...
  /**
   * This test uses the same data as in test testCreateEndPointMasks except that the
   * time between the last data point of the first waveform and the first data point of the
   * second waveform (26 msec) is greater than the sample periods of either waveform (25 msec).
   * The correct result is that no spikes are discovered.
   */
  @Test
   ...
   * (testCreateEndPointMasks) such that the mask on the last point of the first
   * waveform is not created but the spike on the first point of the
   * second waveform is still defined.
   */
  @Test
  ...
  public void testSawtoothOneSpike() throws Exception {
    ...
    assertEquals(sawtooth.getProcessingChannelId(), mask.getChannelId());
