...
public class WaveformSpike3PtInterpreter {
   ...
   * @throws IllegalArgumentException if minConsecutiveSampleDifferenceSpikeThreshold <= 0; if
   * rmsLeadSampleDifferences < 0; if rmsLagSampleDifferences < 0; if (rmsLeadSampleDifferences +
  ...
  public List<WaveformSpike3PtQcMask> createWaveformSpike3PtQcMasks(
      ChannelSegment channelSegment, double minConsecutiveSampleDifferenceSpikeThreshold,
    ...
    logger.info("Perfoming QC spike algorithm on " + channelSegment.getName() + " with threshold "
    ...
    final Function<Instant, WaveformSpike3PtQcMask> createMask = i -> WaveformSpike3PtQcMask
        .create(channelSegment.getProcessingChannelId(), channelSegment.getId(), i);
  ...
  /**
   * Create lists of the adjacent {@link Waveform}s from a {@link ChannelSegment}.  Each Waveform
   * is in exactly one list.  Each list is ordered by Waveform start time.
   *
   ...
   */
  private static List<List<Waveform>> groupAdjacentWaveforms(ChannelSegment channelSegment) {
    ...
    for (Waveform wf : channelSegment.getWaveforms()) {
   ...
   * defined (not null) and the time separation between the last point of lastWaveform and the
   * first point of waveform is <= the maximum waveform sample period of the two input waveforms
  ...
  /**
   * Combines all of the samples from the waveforms into a single double[]. Creates a new copy
   * of the data.
   *
  ...
  private static Collection<Integer> findSpikes(double[] samples, int rmsLeadSampleDifferences,
    ...
    final int start = rmsLeadSampleDifferences;
    ...
    if (start <= end) {
      ...
      for (int start3pt = start; start3pt <= end; ++start3pt) {
   ...
   * opposite sign and the minimum of the two differences exceeds the spikeThreshold times
   * the maximum of the two input differences.
   *
   ...
   * @throws IllegalStateException if sample is less 0; if sample is >= the total number of samples
  ...
  private static Instant timeForSample(List<Waveform> adjacentWaveforms, int sample) {
    ...
    for (Waveform w : adjacentWaveforms) {
      if (sampleInWf >= w.getValues().length) {
        ...
        sampleTime = w.timeForSample(sampleInWf);
        break;
      }
    }
